...
uniform vec3 uLightDir_vs = vec3(0.0); 
...
void main()
{
    ...
    if (uLightDir_vs == vec3(0.0)) fFragColor = texture *(lightCharacter + 0.7*lightHouse + 0.1*lightPuits + 0.05*lightMagic) ;
    else fFragColor = texture *(lightCharacter*0.6 + 0.05*lightMagic + lightDir) ;
}