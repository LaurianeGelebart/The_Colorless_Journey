...
{  
    ...
    for(auto face : this->_facesGroup ){
        ...
        // std::cout << this->_facesGroup[i].getName() << " - " << materialMap[face.getName()].shininess << "\n";

        glUniform1f(this->_program.uShininess, materialMap[face.getName()].shininess);
        glUniform3fv(this->_program.uKd, 1, glm::value_ptr(materialMap[face.getName()].Ka));
        ...
        glUniform1i(this->_program.uTexture2, 1);
        ...
        // std::cout << this->_facesGroup[i].getVertextCount() << "\n" ;
